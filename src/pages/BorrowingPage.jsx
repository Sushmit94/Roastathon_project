          ...
          <div className="col-span-2">
              ...
              <div className="grid grid-cols-3 gap-3 text-sm">
                {markets.map((m) => (
                  <div key={m.id} className="p-3 border rounded">
                    ...
                    <div className="text-xs text-gray-600">Available: {poolLiquidity[m.id] * 0.75}</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="p-4 border rounded mb-6">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div>
                  <div className="text-xs text-gray-600">Total Collateral</div>
                ...
                <div>
                  <div className="text-xs text-gray-600">Borrow Limit</div>
                  <div className="text-xl font-bold text-blue-600">${borrowLimit.toFixed(2)}</div>
                </div>
              </div>
              ...
              <div>
                <div className="text-xs mb-1">Health Factor: <strong>{health}</strong></div>
                <div className="w-full h-3 bg-gray-200 rounded overflow-hidden">
                  <div 
                    style={{ width: `${liquidationRisk}%` }} 
                    className={`h-3 transition-all ${health === 'Safe' ? 'bg-green-600' : health === 'Moderate' ? 'bg-yellow-400' : 'bg-red-500'}`}
                  ></div>
                </div>
                <div className="text-xs mt-1">Liquidation Risk: <strong>{liquidationRisk}%</strong></div>
              </div>
            </section>
                  ...
                  return (
                    <div key={m.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                      <div>
                        ...
                        <span className="text-xs text-gray-600 ml-2">(${(collateralAmt * m.price).toFixed(2)})</span>
                      </div>
